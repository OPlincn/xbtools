...
#2020/4/15    OPlin      增加了一些脚本,同时更新了一些脚本 Beta中
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin
export PATH
#主界面
mainGUI() {
clear
echo -e "\033[32m===============\033[0m\033[36m 小白工具V0.5 Beta \033[32m===============\033[0m"
echo -e "本脚本最后修改于2020/4/15"
echo -e "作者OPlin | 此脚本的详细介绍在www.oplin.cn/index.php/archives/9"
echo -e "使用的所有脚本 均 ${startblue}位于${endblue}/usr/bin/xbct中 \n\n"
...
echo -e "\033[47;34m "TCP加速"脚本 \033[0m" #蓝底黑字
echo -e "--------------------"
echo -e "${startblue}1${endblue}.安装魔改BBR四合一脚本(Githubのcx9208)"
echo -e "${startblue}2${endblue}.一键安装BBR(秋水逸冰大大)"
echo -e "--------------------\n"
echo -e "\033[43;34m "VPS测试脚本" \033[0m" #黄底蓝字
echo -e "--------------------"
echo -e "${startblue}3${endblue}.Superbench综合VPS测试(oooldking)"
echo -e "${startblue}4${endblue}.Zbench服务器综合测试"
echo -e "${startblue}5${endblue}.Linux综合测试脚本(千影/不包括运行unixbench)"
echo -e "${startblue}6${endblue}.SpeedTest测速脚本(Githubのsivel)"
echo -e "${startblue}7${endblue}.UnixBench一键性能跑分(秋水逸冰)"
echo -e "--------------------\n"
echo -e "${startblue}8${endblue}.返回上一页\n"
...
echo -e "${startblue}2${endblue}.一键安装宝塔V7.1.0(Python3)"
...
test -d /usr/bin/xbct
if [ $? == "1" ]; then {
     mkdir /usr/bin/xbct && cd /usr/bin/xbct
   }
 else
   {
echo -e "脚本${startblue}文件夹${endblue}位于/usr/bin/xbct"; cd /usr/bin/xbct
   }
fi
####################################
#进入主界面
...
break
  ...
  unset onechoice
  ck=0
install_judge &&  wget -N --no-check-certificate https://raw.githubusercontent.com/chiakge/Linux-Server-Bench-Test/master/linuxtest.sh && bash linuxtest.sh
break
  ;;
  "6")#SpeedTest测速脚本
  unset onechoice
...
case ${thirdchoice} in
  ...
  break
  ;;
  "2")#一键安装宝塔V7.1.0 Python3
  unset thirdchoice
  ck=0
install_judge &&  curl -sSO http://download.bt.cn/install/install_panel.sh && bash install_panel.sh
  break
  ;;
  "3")#一键安装宝塔V5.9
  unset thirdchoice
